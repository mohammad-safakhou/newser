...
export default function HighlightsList({ items }: Props) {
  if (!items || items.length === 0) return <div className="text-xs text-slate-500">No highlights.</div>
  return (
  ...
  return (
    <li className="border border-slate-800 rounded-md bg-slate-900/50 hover:border-slate-700 transition">
      <button type="button" onClick={()=>setOpen(o=>!o)} className="w-full text-left px-3 py-2 flex items-start gap-3">
        <div className={`px-2 py-0.5 rounded-full text-xs font-medium ${badgeColor}`}>{(h.type||'').toUpperCase() || 'HL'}</div>
        <div className="flex-1 min-w-0">
          <div className="text-sm font-medium text-slate-200 truncate flex items-center gap-2">
            {h.title || 'Untitled'}
            {h.is_pinned && <span className="text-xs px-1 py-0.5 rounded bg-amber-500/20 text-amber-300 border border-amber-400/30">PIN</span>}
            {h.priority != null && <span className={`text-xs px-1 py-0.5 rounded border ${priorityColor}`}>P{h.priority}</span>}
          </div>
          <div className="text-xs text-slate-500 clamp-2">{h.content || '—'}</div>
        </div>
        <div className="text-xs text-slate-500 pl-2" aria-hidden>{open ? '−' : '+'}</div>
      </button>
      {open && (
        <div className="px-4 pb-4 -mt-1 space-y-3 text-xs">
              ...
              <ul className="space-y-0.5 list-disc pl-5">
                {h.sources.map((s, i) => (
                  <li key={i} className="truncate">
                    {s.url ? <a href={s.url} target="_blank" rel="noopener noreferrer" className="text-brand-300 hover:underline">{s.title || s.url}</a> : (s.title || s.origin || 'Source')}
                  </li>
                ))}
              </ul>
            </div>
