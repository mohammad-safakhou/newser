...
func New(ctx context.Context) (*Store, error) {
    ...
    s := &Store{ DB: db }
    if err := s.ensureSchema(ctx); err != nil { return nil, err }
    return s, nil
}
...
func (s *Store) ListTopics(ctx context.Context, userID string) ([]Topic, error) {
    ...
    for rows.Next() {
        var t Topic
        if err := rows.Scan(&t.ID, &t.UserID, &t.Name, &t.ScheduleCron, &t.CreatedAt); err != nil { return nil, err }
        out = append(out, t)
    }
    return out, rows.Err()
}
...
func (s *Store) ListRuns(ctx context.Context, topicID string) ([]Run, error) {
    ...
    for rows.Next() {
        var r Run
        if err := rows.Scan(&r.ID, &r.Status, &r.StartedAt, &r.FinishedAt, &r.Error); err != nil { return nil, err }
        out = append(out, r)
    }
    return out, rows.Err()
}
...
func (s *Store) GetProcessingResultByID(ctx context.Context, id string) (map[string]interface{}, error) {
    ...
    err := s.DB.QueryRowContext(ctx, `SELECT user_thought, summary, detailed_report, sources, highlights, conflicts,
        ...
        &confidence, &processingTime, &cost, new(int64), &agentsB, &modelsB, &metadataB, &created)
    if err != nil { return nil, err }
    ...
    var v interface{}
    _ = json.Unmarshal(userThoughtB, &v); out["user_thought"] = v
    _ = json.Unmarshal(sourcesB, &v); out["sources"] = v
    _ = json.Unmarshal(highlightsB, &v); out["highlights"] = v
    _ = json.Unmarshal(conflictsB, &v); out["conflicts"] = v
    _ = json.Unmarshal(agentsB, &v); out["agents_used"] = v
    _ = json.Unmarshal(modelsB, &v); out["llm_models_used"] = v
    if len(metadataB) > 0 { _ = json.Unmarshal(metadataB, &v); out["metadata"] = v }
    return out, nil
}
