...
func (n *NewsAPIClient) Search(ctx context.Context, query string, options map[string]interface{}) ([]Source, error) {
    ...
    if endpoint == "" { endpoint = "https://newsapi.org/v2/everything" }
    var resp struct {
        Articles []struct{
    ...
    q := query
    if mq, ok := options["query"].(string); ok && mq != "" { q = mq }
...
func (n *NewsAPIClient) GetSource(ctx context.Context, id string) (Source, error) { return Source{}, fmt.Errorf("not implemented") }
...
func (b *BraveClient) Search(ctx context.Context, query string, options map[string]interface{}) ([]Source, error) {
    var resp struct{
        Web struct{ Results []struct{ Title, URL, Description string } `json:"results"` } `json:"web"`
    }
    ...
    if err := b.http.DoJSON(ctx, "GET", url, headers, nil, &resp); err != nil { return nil, err }
    var out []Source
...
func (b *BraveClient) GetSource(ctx context.Context, id string) (Source, error) { return Source{}, fmt.Errorf("not implemented") }
...
func (s *SerperClient) Search(ctx context.Context, query string, options map[string]interface{}) ([]Source, error) {
    var resp struct{ Organic []struct{ Title, Link, Snippet string } `json:"organic"` }
    headers := map[string]string{"X-API-KEY": s.cfg.SerperAPIKey}
    body := map[string]any{"q": query, "num": max1(s.cfg.MaxResults, 10)}
    if mq, ok := options["query"].(string); ok && mq != "" { body["q"] = mq }
    if err := s.http.DoJSON(ctx, "POST", "https://google.serper.dev/search", headers, body, &resp); err != nil { return nil, err }
    var out []Source
...
func (s *SerperClient) GetSource(ctx context.Context, id string) (Source, error) { return Source{}, fmt.Errorf("not implemented") }
...
func max1(a, def int) int { if a > 0 { return a }; return def }
...
func DeduplicateSources(in []Source) []Source {
    ...
    for _, s := range in {
        ...
        if prev, ok := seen[k]; ok {
            if s.Credibility > prev.Credibility { seen[k] = s }
        } else {
            seen[k] = s
        }
    }
