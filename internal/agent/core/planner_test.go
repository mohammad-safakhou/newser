...
import (
	...
	agentcfg "github.com/mohammad-safakhou/newser/config"
)

type dummyLLM struct{}

func (d *dummyLLM) Generate(ctx context.Context, prompt string, model string, options map[string]interface{}) (string, error) {
	return "", nil
}
func (d *dummyLLM) GenerateWithTokens(ctx context.Context, prompt string, model string, options map[string]interface{}) (string, int64, int64, error) {
	return "", 0, 0, nil
}
...
func TestPlanner_ParsePlanningResponse_CoerceNumericIDs(t *testing.T) {
	...
	if plan.Tasks[0].ID == "" {
		t.Fatalf("expected coerced string ID, got empty")
	}
}
