...
func Run(addr string) error {
    ...
    if cfg, err := agentcfg.LoadConfig(); err == nil {
        ...
        if os.Getenv("DATABASE_URL") == "" && (cfg.Storage.Postgres.URL != "" || cfg.Storage.Postgres.Host != "") {
            if cfg.Storage.Postgres.URL != "" {
                ...
                if cfg.Storage.Postgres.Host != "" { os.Setenv("POSTGRES_HOST", cfg.Storage.Postgres.Host) }
                if cfg.Storage.Postgres.Port != 0 { os.Setenv("POSTGRES_PORT", fmt.Sprintf("%d", cfg.Storage.Postgres.Port)) }
                if cfg.Storage.Postgres.User != "" { os.Setenv("POSTGRES_USER", cfg.Storage.Postgres.User) }
                if cfg.Storage.Postgres.Password != "" { os.Setenv("POSTGRES_PASSWORD", cfg.Storage.Postgres.Password) }
                if cfg.Storage.Postgres.DBName != "" { os.Setenv("POSTGRES_DB", cfg.Storage.Postgres.DBName) }
                if cfg.Storage.Postgres.SSLMode != "" { os.Setenv("POSTGRES_SSLMODE", cfg.Storage.Postgres.SSLMode) }
            }
        }
    ...
    auth, err := initAuth(ctx, []byte(srvCfg.JWTSecret))
    ...
    me.GET("", func(c echo.Context) error { return c.JSON(200, map[string]string{"user_id": c.Get("user_id").(string)}) })

    // topics endpoints
    th := &TopicsHandler{ Store: auth.Store }
    th.Register(api.Group("/topics"), auth.Secret)

    rh := &RunsHandler{ Store: auth.Store }
