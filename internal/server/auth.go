...
func (a *AuthHandler) signup(c echo.Context) error {
    ...
    if err := a.Store.CreateUser(c.Request().Context(), req.Email, string(hash)); err != nil { return echo.NewHTTPError(http.StatusConflict, err.Error()) }
...
func (a *AuthHandler) login(c echo.Context) error {
    ...
    if time.Since(lastLoginAttempt) < 300*time.Millisecond { return echo.NewHTTPError(http.StatusTooManyRequests, "too many requests") }
    lastLoginAttempt = time.Now()

    var req struct{ Email, Password string }
    if err := c.Bind(&req); err != nil { return echo.NewHTTPError(http.StatusBadRequest, err.Error()) }
    if len(req.Password) < 8 { return echo.NewHTTPError(http.StatusBadRequest, "password too short") }
    ...
    if bcrypt.CompareHashAndPassword([]byte(hash), []byte(req.Password)) != nil { return echo.NewHTTPError(http.StatusUnauthorized, "invalid credentials") }
...
func withAuth(next echo.HandlerFunc, secret []byte) echo.HandlerFunc {
    return func(c echo.Context) error {
        ...
        if tok == "" {
            ...
            if err != nil { return echo.NewHTTPError(http.StatusUnauthorized, "missing auth cookie") }
            tok = ck.Value
        }
        token, err := jwt.Parse(tok, func(t *jwt.Token) (interface{}, error) { return secret, nil })
        if err != nil || !token.Valid { return echo.NewHTTPError(http.StatusUnauthorized, "invalid token") }
...
func initAuth(ctx context.Context, st *store.Store, jwtSecret []byte) (*AuthHandler, error) {
    if st == nil { return nil, echo.NewHTTPError(http.StatusInternalServerError, "store not initialized") }
